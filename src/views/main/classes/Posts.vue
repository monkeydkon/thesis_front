<template>
  <div class="d-flex flex-column pb-12">
    <v-btn
      ...
      >Create a post</v-btn
    >
    <div v-if="selectedClass.posts.length > 0">
      <div v-for="post in selectedClass.posts" :key="post.id" class="mt-5">
  ...
  methods: {
    submit() {
      ...
      if (!this.$v.$invalid) {
        ...
        this.$store
          .dispatch("teacherNewPost", credentials)
          .then((res) => {
